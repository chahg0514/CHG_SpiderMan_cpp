...
struct MeshMerging {
/**
 * Mesh merging modes for non-instanced meshes. Note that meshes of different type
 * are never merged, i.e. a mesh can only have one type of polygon or edge or vertex faces.
 */
enum Action {
	NONE,							///< Meshes are not merged.
	ALL_OF_SAME_MATERIAL_AND_TYPE,	///< Non-instanced meshes of the same type and with identical material attributes are merged together.
	ALL_OF_SAME_TYPE				///< All non-instanced meshes of the same type are merged together.
};
};
...
public:
	...
	public:
		 ...
		 * materials attached to the meshes are the original asset materials.
		 *
		 * The size and order of the returned vector corresponds to the concatenation
		 * of all materials returned by prtx::Mesh::getMaterials().
		 *
		 * \sa PreparationFlags::instancing
		 * \sa prtx::Mesh::getMaterials
		 */
	...
	public:
		 ...
		 */
		[[deprecated("replaced by meshMerging")]] PreparationFlags& mergeByMaterial(bool v);
		...
		[[deprecated("replaced by meshMerging")]] bool mergeByMaterial() const;
	...
	private:
		bool mInstancing;
		MeshMerging::Action mMeshMerging;
