...
private:
	...
	void StopWebSwingingFromJump(); //스윙중에 점프했을 때 스윙 끝내고, 스윙점프 true
	...
	FVector SwingPoint;
	bool IsTheHitInFrontOfThePlayer(FVector Point); //트레이스 Hit이 플레이어보다 적당히 위에 있는지
...
private:
	...
	void PlayNextCombo();
	void PlayAttackMontage(int comboCount);
	void WhenAttackFunc();
